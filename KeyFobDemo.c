...
#include "KeyFobDemo.h"          /* Application Header.                       */
...
#include "SS1BTKFS.h"            /* Main Key Fob Header.                      */
...
#include "Buttons.h"             /* Buttons Hardware Module Header.           */
...
typedef struct _tagApplicationStateInfo_t
{
   ...
   KFS_Key_Pressed_State_t CurrentKeyState;
...
typedef struct _tagDeviceInfo_t
{
   ...
   Word_t                       EDIV;
   KFS_Server_Information_t     KFSServerInformation;
...
typedef char BoardStr_t[16];
                        ...
                        /* entropy.                                     */
static BTPSCONST Encryption_Key_t ER = {0x28, 0xBA, 0xE1, 0x35, 0x13, 0xB2, 0x20, 0x45, 0x16, 0xB2, 0x19, 0xD0, 0x80, 0xEE, 0x4A, 0x51};

                        /* The Identity Root Key should be generated    */
                        /* in such a way as to guarantee 128 bits of    */
                        /* entropy.                                     */
static BTPSCONST Encryption_Key_t IR = {0x41, 0x09, 0xA2, 0x88, 0x09, 0x6B, 0x70, 0xC0, 0x95, 0x23, 0x3C, 0x8C, 0x48, 0xFC, 0xC9, 0xFE};
...
static void FormatAdvertisingData(unsigned int BluetoothStackID, Boolean_t SupportBR_EDR);

static int StartAdvertising(unsigned int BluetoothStackID);

static void IdleFunction(unsigned int BluetoothStackID);

static unsigned int FormatSPPDataPacket(unsigned int PacketBufferLength, Byte_t *PacketBuffer, KFS_Key_Pressed_State_t *KeyState, Accelerometer_Data_t *AccelerometerData);
static void ProcessSendSPPData(Boolean_t PacketizeCurrentData);
static void ProcessSendLEKeyStateData(void);
...
static void ButtonPressCallback(Button_t Button, unsigned char ButtonState);
...
static void BTPSAPI KFS_Event_Callback(unsigned int BluetoothStackID, KFS_Event_Data_t *KFS_Event_Data, unsigned long CallbackParameter);
static void BTPSAPI ACC_Event_Callback(unsigned int BluetoothStackID, ACC_Event_Data_t *ACC_Event_Data, unsigned long CallbackParameter);
...
static int SPPOpenServer(unsigned int BluetoothStackID)
{
   ...
   if(BluetoothStackID)
   {
      ...
      /* 1.                                                             */
      ret_val = SPP_Open_Server_Port(BluetoothStackID, SPP_PORT_NUMBER, SPP_Event_Callback, (unsigned long)0);
      ...
      if(ret_val > 0)
      {
         ...
         if((ServiceName = BTPS_AllocateMemory(64)) != NULL)
         {
            ...
            BTPS_SprintF(ServiceName, "Serial Port Server Port %d", SPP_PORT_NUMBER);

            /* Now that a Service Name has been created try to Register */
            /* the SDP Record.                                          */
            ret_val = SPP_Register_SDP_Record(BluetoothStackID, ApplicationStateInfo.SPPServerPortID, NULL, ServiceName, &(ApplicationStateInfo.SPPServerSDPHandle));
            ...
            if(ret_val < 0)
            {
               ...
               SPP_Close_Server_Port(BluetoothStackID, ApplicationStateInfo.SPPServerPortID);
...
static int OpenStack(HCI_DriverInformation_t *HCI_DriverInformation, BTPS_Initialization_t *BTPS_Initialization)
{
   ...
   if(HCI_DriverInformation)
   {
      ...
      if(Result > 0)
      {
         ...
         /* (Interlaced scanning).                                      */
         HCI_Write_Inquiry_Scan_Type(ApplicationStateInfo.BluetoothStackID, HCI_INQUIRY_SCAN_TYPE_OPTIONAL_INTERLACED_SCAN, &Status);
         HCI_Write_Page_Scan_Type(ApplicationStateInfo.BluetoothStackID, HCI_PAGE_SCAN_TYPE_OPTIONAL_INTERLACED_SCAN, &Status);
         ...
         if(!(Result = SPPOpenServer(ApplicationStateInfo.BluetoothStackID)))
         {
            ...
            if(!(Result = GATT_Initialize(ApplicationStateInfo.BluetoothStackID, GATT_INITIALIZATION_FLAGS_SUPPORT_LE, GATT_Connection_Event_Callback, 0)))
            {
               ...
               if(Result > 0)
               {
                  ...
                  GAPS_Set_Device_Appearance(ApplicationStateInfo.BluetoothStackID, ApplicationStateInfo.GAPSInstanceID, GAP_DEVICE_APPEARENCE_VALUE_GENERIC_COMPUTER);
   
                  /* Intialize the KFS Service.                         */
                  Result = KFS_Initialize_Service(ApplicationStateInfo.BluetoothStackID, KFS_Event_Callback, 0, &ServiceID);
                  if(Result > 0)
                  {
                     /* Save the Instance ID of the KFS Service.        */
                     ApplicationStateInfo.KFSInstanceID = (unsigned int)Result;
   
                     /* Set the Key Press Characteristic User           */
                     /* Description String.                             */
                     KFS_Set_Key_Press_User_Description(ApplicationStateInfo.BluetoothStackID, ApplicationStateInfo.KFSInstanceID, "KeyPress-ez430");
   
                     /* Initialize the ACC Service.                     */
                     Result = ACC_Initialize_Service(ApplicationStateInfo.BluetoothStackID, ACC_Event_Callback, 0, &ServiceID);
                     if(Result > 0)
                     {
                        /* Save the Instance ID of the ACC Service.     */
                        ApplicationStateInfo.ACCInstanceID = (unsigned int)Result;
   
                        /* Set the Range Characteristic of the          */
                        /* Accelerometer Service.                       */
                        ACC_Set_Accelerometer_Range(ApplicationStateInfo.BluetoothStackID, ApplicationStateInfo.ACCInstanceID, TI_ACC_RANGE_2G);

                        /* Save the callback handle.                    */
                        ApplicationStateInfo.HCIEventCallbackHandle = (unsigned int)ret_val;
   
                        /* Format the EIR Data.                         */
                        FormatEIRData(ApplicationStateInfo.BluetoothStackID);
   
                        /* Format the Advertising Data.                 */
                        FormatAdvertisingData(ApplicationStateInfo.BluetoothStackID, TRUE);
                     ...
                     else
                     {
                        ...
                        DisplayFunctionError("ACC_Initialize_Service", Result);
                  ...
                  else
                  {
                     ...
                     DisplayFunctionError("KFS_Initialize_Service", Result);
...
static int CloseStack(void)
{
   ...
   if(ApplicationStateInfo.BluetoothStackID)
   {
      ...
      /* Clean up the KFS Service Module.                               */
      if(ApplicationStateInfo.KFSInstanceID)
         KFS_Cleanup_Service(ApplicationStateInfo.BluetoothStackID, ApplicationStateInfo.KFSInstanceID);

      /* Clean up the ACC Service Module.                               */
      if(ApplicationStateInfo.ACCInstanceID)
         ACC_Cleanup_Service(ApplicationStateInfo.BluetoothStackID, ApplicationStateInfo.ACCInstanceID);
      ...
      if(ApplicationStateInfo.HCIEventCallbackHandle)
         HCI_Un_Register_Callback(ApplicationStateInfo.BluetoothStackID, ApplicationStateInfo.HCIEventCallbackHandle);
...
static void FormatEIRData(unsigned int BluetoothStackID)
{
   ...
   if(BluetoothStackID)
   {
      ...
      *TempPtr++ = HCI_EXTENDED_INQUIRY_RESPONSE_DATA_TYPE_16_BIT_SERVICE_CLASS_UUID_COMPLETE;
      ...
      TI_KFS_ASSIGN_KFS_SERVICE_UUID_16(*((UUID_16_t *)TempPtr));
...
static void FormatAdvertisingData(unsigned int BluetoothStackID, Boolean_t SupportBR_EDR)
{
   ...
   if(BluetoothStackID)
   {
      ...
      Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[Length] = 1 + (UUID_16_SIZE<<1);
      ...
      TI_KFS_ASSIGN_KFS_SERVICE_UUID_16(*((UUID_16_t *)&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[Length+2])));
      TI_ACC_ASSIGN_ACC_SERVICE_UUID_16(*((UUID_16_t *)&(Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[Length+4])));
...
static unsigned int FormatSPPDataPacket(unsigned int PacketBufferLength, Byte_t *PacketBuffer, KFS_Key_Pressed_State_t *KeyState, Accelerometer_Data_t *AccelerometerData)
{
   ...
   if((PacketBufferLength >= KEY_FOB_SPP_BUTTON_ACCEL_DATA_SIZE) && (PacketBuffer) && (KeyState) && (AccelerometerData))

#else

   if((PacketBufferLength >= KEY_FOB_SPP_BUTTON_ACCEL_DATA_SIZE+6) && (PacketBuffer) && (KeyState) && (AccelerometerData))

#endif
   {
      /* Convert the Key Press State to binary data.                    */
      Temp = 0;
      if(KeyState->KeyOnePressed)
         Temp |= TI_KFS_KEY_PRESSED_BUTTON_ONE_PRESSED;

      if(KeyState->KeyTwoPressed)
         Temp |= TI_KFS_KEY_PRESSED_BUTTON_TWO_PRESSED;

#ifndef ASCII_DATA
      
      /* Format the packet.                                             */
      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&(((Key_Fob_SPP_Button_Accel_Data_t *)PacketBuffer)->Header.PacketType), KEY_FOB_SPP_PACKET_TYPE_BUTTON_ACCEL_DATA);
      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&(((Key_Fob_SPP_Button_Accel_Data_t *)PacketBuffer)->Header.PacketLength), (KEY_FOB_SPP_BUTTON_ACCEL_DATA_SIZE - KEY_FOB_SPP_HEADER_DATA_SIZE));
      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&(((Key_Fob_SPP_Button_Accel_Data_t *)PacketBuffer)->KeyPressData), Temp);
      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&(((Key_Fob_SPP_Button_Accel_Data_t *)PacketBuffer)->X_Axis), AccelerometerData->X_Axis);
      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&(((Key_Fob_SPP_Button_Accel_Data_t *)PacketBuffer)->Y_Axis), AccelerometerData->Y_Axis);
      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&(((Key_Fob_SPP_Button_Accel_Data_t *)PacketBuffer)->Z_Axis), AccelerometerData->Z_Axis);

      ret_val = KEY_FOB_SPP_BUTTON_ACCEL_DATA_SIZE;

#else

      /* Format the packet.                                             */
     //BTPS_SprintF((char *)PacketBuffer, "%02X:%02X:%02X:%02X:%02X:%02X", TISKIT_SPP_PACKET_TYPE_BUTTON_ACCEL_DATA, (KEY_FOB_SPP_BUTTON_ACCEL_DATA_SIZE - TISKIT_SPP_HEADER_DATA_SIZE), Temp, 
     //                                                             AccelerometerData->X_Axis, AccelerometerData->Y_Axis, AccelerometerData->Z_Axis);
      
      BTPS_SprintF((char *)PacketBuffer, "Buttons:%02X, accX:%02X, accY:%02X, accZ:%02X\n\r", Temp, 
                                                                  AccelerometerData->X_Axis, AccelerometerData->Y_Axis, AccelerometerData->Z_Axis);

      ret_val = BTPS_StringLength((char *)PacketBuffer);
...
static void ProcessSendSPPData(Boolean_t PacketizeCurrentData)
{
   ...
   if((ApplicationStateInfo.Flags & (APPLICATION_STATE_INFO_FLAGS_CB_CONNECTED | APPLICATION_STATE_INFO_FLAGS_SPP_BUFFER_FULL)) == APPLICATION_STATE_INFO_FLAGS_CB_CONNECTED)
   {
      ...
      if(PacketizeCurrentData)
      {
         ...
         ApplicationStateInfo.SPPBufferLength += FormatSPPDataPacket(SPP_BUFFER_SIZE - ApplicationStateInfo.SPPBufferLength, &(ApplicationStateInfo.SPPBuffer[ApplicationStateInfo.SPPBufferLength]), &(ApplicationStateInfo.CurrentKeyState), &(ApplicationStateInfo.CurrentAccelData));
      }

      /* Send the packetized SPP data to the remote device.             */
      Result = SPP_Data_Write(ApplicationStateInfo.BluetoothStackID, ApplicationStateInfo.SPPServerPortID, ApplicationStateInfo.SPPBufferLength, (unsigned char *)ApplicationStateInfo.SPPBuffer);
      ...
      if(Result > 0)
      {
         ...
         {
            BTPS_MemMove(ApplicationStateInfo.SPPBuffer, &(ApplicationStateInfo.SPPBuffer[(unsigned int)Result]), (ApplicationStateInfo.SPPBufferLength - Result));

            ApplicationStateInfo.Flags |= APPLICATION_STATE_INFO_FLAGS_SPP_BUFFER_FULL;
         }
   ...
   /* The following function is a utility function that is used to      */
   /* process and send Key State information over the LE transport using*/
   /* GATT.                                                             */
static void ProcessSendLEKeyStateData(void)
{
   int           Result;
   ...
   if(ApplicationStateInfo.Flags & APPLICATION_STATE_INFO_FLAGS_LE_CONNECTED)
   {
      ...
      DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ApplicationStateInfo.LEConnectionInfo.BD_ADDR);
      if(DeviceInfo)
      {
         /* Verified that the selected client has configured the Key    */
         /* State Characteristic for notifications.                     */
         if(DeviceInfo->KFSServerInformation.Key_Press_Client_Configuration)
         {
            Result = KFS_Notify_Key_Press_State(ApplicationStateInfo.BluetoothStackID, ApplicationStateInfo.KFSInstanceID, ApplicationStateInfo.LEConnectionInfo.ConnectionIndex, &(ApplicationStateInfo.CurrentKeyState));
            if(!Result)
               Display(("KFS_Notify_Key_Press_State success.\r\n"));
            else
               Display(("Error - KFS_Notify_Key_Press_State returned %d.\r\n", Result));
         }
      }
      else
...
static void ProcessReadAccelData(void)
{
   ...
   if(ApplicationStateInfo.AccelEnableCount)
   {
      ...
      if(Accelerometer_Ready())
      {
         ...
         if((ApplicationStateInfo.CurrentAccelData.X_Axis < AccelData.X_Axis-1) || (ApplicationStateInfo.CurrentAccelData.X_Axis > AccelData.X_Axis+1))
         {
         ...
         if((ApplicationStateInfo.CurrentAccelData.Y_Axis < AccelData.Y_Axis-1) || (ApplicationStateInfo.CurrentAccelData.Y_Axis > AccelData.Y_Axis+1))
         {
         ...
         if((ApplicationStateInfo.CurrentAccelData.Z_Axis < AccelData.Z_Axis-1) || (ApplicationStateInfo.CurrentAccelData.Z_Axis > AccelData.Z_Axis+1))
         {
...
static void ProcessSendLEAccelData(Byte_t AxisChangeMask)
{
   ...
   if(ApplicationStateInfo.Flags & APPLICATION_STATE_INFO_FLAGS_LE_CONNECTED)
   {
      ...
      if(DeviceInfo)
      {
         ...
         if((AxisChangeMask & APPLICATION_MAILBOX_MESSAGE_ID_X_AXIS) && (DeviceInfo->ACCServerInformation.XAxis_Client_Configuration))
         {
            Result = ACC_Notify_Axis(ApplicationStateInfo.BluetoothStackID, ApplicationStateInfo.ACCInstanceID, ApplicationStateInfo.LEConnectionInfo.ConnectionIndex, ctXAxis, ApplicationStateInfo.CurrentAccelData.X_Axis);
            if(!Result)
               NotifiedMask |= APPLICATION_MAILBOX_MESSAGE_ID_X_AXIS;
            else
               Display(("Error - ACC_Notify_Axis ctXAxis %d.\r\n", Result));
         }

         if((AxisChangeMask & APPLICATION_MAILBOX_MESSAGE_ID_Y_AXIS) && (DeviceInfo->ACCServerInformation.YAxis_Client_Configuration))
         {
            Result = ACC_Notify_Axis(ApplicationStateInfo.BluetoothStackID, ApplicationStateInfo.ACCInstanceID, ApplicationStateInfo.LEConnectionInfo.ConnectionIndex, ctYAxis, ApplicationStateInfo.CurrentAccelData.Y_Axis);
            if(!Result)
               NotifiedMask |= APPLICATION_MAILBOX_MESSAGE_ID_Y_AXIS;
            else
               Display(("Error - ACC_Notify_Axis ctYAxis %d.\r\n", Result));
         }

         if((AxisChangeMask & APPLICATION_MAILBOX_MESSAGE_ID_Z_AXIS) && (DeviceInfo->ACCServerInformation.ZAxis_Client_Configuration))
         {
            Result = ACC_Notify_Axis(ApplicationStateInfo.BluetoothStackID, ApplicationStateInfo.ACCInstanceID, ApplicationStateInfo.LEConnectionInfo.ConnectionIndex, ctZAxis, ApplicationStateInfo.CurrentAccelData.Z_Axis);
            if(!Result)
               NotifiedMask |= APPLICATION_MAILBOX_MESSAGE_ID_Z_AXIS;
            else
               Display(("Error - ACC_Notify_Axis ctZAxis %d.\r\n", Result));
         }
   ...
   /* The following function is the button press callback which is      */
   /* called whenever a button is pressed.                              */
static void ButtonPressCallback(Button_t Button, unsigned char ButtonState)
{
   Boolean_t Pressed;
   if((Button == btSwitch1) || (Button == btSwitch2))
   {
      Display(("Button Press %u %u.\r\n", (unsigned int)Button, (unsigned int)ButtonState));

      /* Convert Button State to a Boolean.                             */
      if(ButtonState == BUTTON_STATE_BUTTON_RELEASED)
         Pressed = FALSE;
      else
         Pressed = TRUE;
      ...
      if(Button == btSwitch1)
         ApplicationStateInfo.CurrentKeyState.KeyOnePressed = Pressed;
      else
         ApplicationStateInfo.CurrentKeyState.KeyTwoPressed = Pressed;

      /* Post a message to the event handler.                           */
      PostApplicationMailbox(APPLICATION_MAILBOX_MESSAGE_ID_KEY_STATE_CHANGE);
   }
}
               ...
               if(GAP_LE_Event_Data->Event_Data.GAP_LE_Connection_Complete_Event_Data->Status == HCI_ERROR_CODE_NO_ERROR)
               {
                  ...
                  else
                  {
                     ...
                     if(DeviceInfo->ACCServerInformation.AccelerometerEnabled)
                        PostApplicationMailbox(APPLICATION_MAILBOX_MESSAGE_ID_ACCEL_ENABLE);
                  }
               }
            }
            break;
         case etLE_Disconnection_Complete:
            ...
            if(GAP_LE_Event_Data->Event_Data.GAP_LE_Disconnection_Complete_Event_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GAP_LE_Event_Data->Event_Data.GAP_LE_Disconnection_Complete_Event_Data->Peer_Address)) != NULL)
               {
                  ...
                  if(DeviceInfo->ACCServerInformation.AccelerometerEnabled)
                     PostApplicationMailbox(APPLICATION_MAILBOX_MESSAGE_ID_ACCEL_DISABLE);

                  /* Check to see if the link is encrypted.  If it isn't*/
                  /* we will delete the device structure.               */
                  Result = GAP_LE_Query_Encryption_Mode(BluetoothStackID, GAP_LE_Event_Data->Event_Data.GAP_LE_Disconnection_Complete_Event_Data->Peer_Address, &GAP_Encryption_Mode);
                  if((Result) || (GAP_Encryption_Mode == emDisabled))
                  {
                     /* Connection is not encrypted so delete the device*/
                     /* structure.                                      */
                     DeviceInfo = DeleteDeviceInfoEntry(&DeviceInfoList, GAP_LE_Event_Data->Event_Data.GAP_LE_Disconnection_Complete_Event_Data->Peer_Address);
                     if(DeviceInfo)
               ...
               PostApplicationMailbox(APPLICATION_MAILBOX_MESSAGE_ID_LE_DISCONNECTED);
            }
            break;
      ...
      switch(GATT_Connection_Event_Data->Event_Data_Type)
      {
         case etGATT_Connection_Device_Connection:
            if(GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data)
            {
               ...
               ApplicationStateInfo.LEConnectionInfo.ConnectionIndex = GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data->ConnectionID;
               ApplicationStateInfo.LEConnectionInfo.BD_ADDR          = GATT_Connection_Event_Data->Event_Data.GATT_Device_Connection_Data->RemoteDevice;

               /* Notify the event handler of the connection.           */
               PostApplicationMailbox(APPLICATION_MAILBOX_MESSAGE_ID_LE_CONNECTED);
            }
            else
...
static void BTPSAPI KFS_Event_Callback(unsigned int BluetoothStackID, KFS_Event_Data_t *KFS_Event_Data, unsigned long CallbackParameter)
{
   ...
   /* semi-valid.                                                       */
   if((BluetoothStackID) && (KFS_Event_Data))
   {
      switch(KFS_Event_Data->Event_Data_Type)
      {
         case etKFS_Server_Read_Client_Configuration_Request:
            if(KFS_Event_Data->Event_Data.KFS_Read_Client_Configuration_Data)
            {
               /* Search the device list for the connection.            */
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, KFS_Event_Data->Event_Data.KFS_Read_Client_Configuration_Data->RemoteDevice)) != NULL)
                  NotificationsEnabled = DeviceInfo->KFSServerInformation.Key_Press_Client_Configuration;
               else
                  NotificationsEnabled = FALSE;

               Result = KFS_Read_Client_Configuration_Response(BluetoothStackID, KFS_Event_Data->Event_Data.KFS_Read_Client_Configuration_Data->InstanceID, KFS_Event_Data->Event_Data.KFS_Read_Client_Configuration_Data->TransactionID, NotificationsEnabled);
               if(Result)
                  Display(("Error - KFS_Read_Client_Configuration_Response returned %d.\r\n", Result));
            }  
            break;
         case etKFS_Server_Client_Configuration_Update:
            if(KFS_Event_Data->Event_Data.KFS_Client_Configuration_Update_Data)
            {
               /* Search the device list for the connection.            */
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, KFS_Event_Data->Event_Data.KFS_Client_Configuration_Update_Data->RemoteDevice)) != NULL)
                  DeviceInfo->KFSServerInformation.Key_Press_Client_Configuration = KFS_Event_Data->Event_Data.KFS_Client_Configuration_Update_Data->NotificationsEnabled;
            }  
            break;
      }
   }
   else
   {
      ...
      Display(("KFS Event Callback Data: Event_Data = NULL.\r\n"));
   }
}
   ...
   if((BluetoothStackID) && (ACC_Event_Data))
   {
      switch(ACC_Event_Data->Event_Data_Type)
      {
         case etACC_Server_Read_Client_Configuration_Request:
            ...
            {
               /* Search the device list for the connection.            */
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ACC_Event_Data->Event_Data.ACC_Read_Client_Configuration_Data->RemoteDevice)) != NULL)
               {
                  switch(ACC_Event_Data->Event_Data.ACC_Read_Client_Configuration_Data->CharacteristicType)
                  {
                     case ctXAxis:
                        NotificationsEnabled = DeviceInfo->ACCServerInformation.XAxis_Client_Configuration;
                        break;
                     case ctYAxis:
                        NotificationsEnabled = DeviceInfo->ACCServerInformation.YAxis_Client_Configuration;
                        break;
                     case ctZAxis:
                        NotificationsEnabled = DeviceInfo->ACCServerInformation.ZAxis_Client_Configuration;
                        break;
                     default:
                        NotificationsEnabled = FALSE;
                        break;
               ...
               Result = ACC_Read_Client_Configuration_Response(BluetoothStackID, ACC_Event_Data->Event_Data.ACC_Read_Client_Configuration_Data->InstanceID, ACC_Event_Data->Event_Data.ACC_Read_Client_Configuration_Data->TransactionID, NotificationsEnabled);
               if(Result)
                  Display(("Error - ACC_Read_Client_Configuration_Response returned %d.\r\n", Result));
            }  
            break;
         case etACC_Server_Client_Configuration_Update:
            ...
            {
               /* Search the device list for the connection.            */
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ACC_Event_Data->Event_Data.ACC_Client_Configuration_Update_Data->RemoteDevice)) != NULL)
               {
                  switch(ACC_Event_Data->Event_Data.ACC_Client_Configuration_Update_Data->CharacteristicType)
                  {
                     case ctXAxis:
                        DeviceInfo->ACCServerInformation.XAxis_Client_Configuration = ACC_Event_Data->Event_Data.ACC_Client_Configuration_Update_Data->NotificationsEnabled;
                        break;
                     case ctYAxis:
                        DeviceInfo->ACCServerInformation.YAxis_Client_Configuration = ACC_Event_Data->Event_Data.ACC_Client_Configuration_Update_Data->NotificationsEnabled;
                        break;
                     case ctZAxis:
                        DeviceInfo->ACCServerInformation.ZAxis_Client_Configuration = ACC_Event_Data->Event_Data.ACC_Client_Configuration_Update_Data->NotificationsEnabled;
                        break;
                     default:
                        /* Do nothing.                                  */
                        break;
         ...
         case etACC_Server_Accelerometer_Enable_Read_Request:
            ...
            {
               /* Search the device list for the connection.            */
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ACC_Event_Data->Event_Data.ACC_Accelerometer_Enable_Read_Request_Data->RemoteDevice)) != NULL)
                  NotificationsEnabled = DeviceInfo->ACCServerInformation.AccelerometerEnabled;
               else
                  NotificationsEnabled = FALSE;

               Result = ACCEL_Enable_Read_Request_Response(BluetoothStackID, ACC_Event_Data->Event_Data.ACC_Accelerometer_Enable_Read_Request_Data->InstanceID, ACC_Event_Data->Event_Data.ACC_Accelerometer_Enable_Read_Request_Data->TransactionID, NotificationsEnabled);
               if(Result)
                  Display(("Error - ACCEL_Enable_Read_Request_Response returned %d.\r\n", Result));
            }
            break;
         case etACC_Server_Accelerometer_Enable_Update_Request:
            ...
            {
               /* Search the device list for the connection.            */
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ACC_Event_Data->Event_Data.ACC_Accelerometer_Enable_Update_Request_Data->RemoteDevice)) != NULL)
               {
                  DeviceInfo->ACCServerInformation.AccelerometerEnabled = ACC_Event_Data->Event_Data.ACC_Accelerometer_Enable_Update_Request_Data->EnableAccelerometer;
               }

               Result = ACCEL_Enable_Update_Request_Response(BluetoothStackID, ACC_Event_Data->Event_Data.ACC_Accelerometer_Enable_Read_Request_Data->InstanceID, ACC_Event_Data->Event_Data.ACC_Accelerometer_Enable_Read_Request_Data->TransactionID, 0);
               if(!Result)
               {
         ...
         case etAuthentication:
            ...
            switch(GAP_Event_Data->Event_Data.GAP_Authentication_Event_Data->GAP_Authentication_Event_Type)
            {
               ...
               case atPINCodeRequest:
                  ...
                  BTPS_MemCopy(&PINCode, CLASSIC_PIN_CODE, BTPS_StringLength(CLASSIC_PIN_CODE));

                  /* Populate the response structure.                   */
                  GAP_Authentication_Information.GAP_Authentication_Type      = atPINCode;
                  GAP_Authentication_Information.Authentication_Data_Length   = (Byte_t)(BTPS_StringLength(CLASSIC_PIN_CODE));
               ...
               case atAuthenticationStatus:
                  ...
                  if(GAP_Event_Data->Event_Data.GAP_Authentication_Event_Data->Authentication_Event_Data.Authentication_Status)
                  {
                     ...
                     /* any stored link key.                            */
                     DeleteLinkKey(BluetoothStackID, GAP_Event_Data->Event_Data.GAP_Authentication_Event_Data->Remote_Device);
                  }
                  break;
   ...
   if((SPP_Event_Data) && (BluetoothStackID))
   {
      ...
      switch(SPP_Event_Data->Event_Data_Type)
      {
         case etPort_Open_Indication:
            ...
            /* Store the Connection Information for this connection.    */
            ApplicationStateInfo.CBConnectionInfo.BD_ADDR = SPP_Event_Data->Event_Data.SPP_Open_Port_Indication_Data->BD_ADDR;
         ...
         case etPort_Close_Port_Indication:
            ...
            PostApplicationMailbox(APPLICATION_MAILBOX_MESSAGE_ID_CB_DISCONNECTED);
            break;
         ...
         case etPort_Transmit_Buffer_Empty_Indication:
            ...
            PostApplicationMailbox(APPLICATION_MAILBOX_MESSAGE_ID_SPP_BUFFER_EMPTY);
            break;
         default:
   ...
   if((HCI_DriverInformation) && (BTPS_Initialization))
   {
      ...
      if(!OpenStack(HCI_DriverInformation, BTPS_Initialization))
      {
         ...
         if(!ret_val)
         {
            ...
            {
               /* Initialize the button handling module.                */
               ButtonInit(ButtonPressCallback);
   ...
   if(ApplicationStateInfo.Mailbox)
   {
      ...
      while(1)
      {
         ...
         if(BTPS_WaitMailbox(ApplicationStateInfo.Mailbox, &MessageID))
         {
            switch(MessageID)
            {
               case APPLICATION_MAILBOX_MESSAGE_ID_KEY_STATE_CHANGE:
                  /* Key Press Data has changed so we need to send the  */
                  /* current information to any connected device.       */
                  ProcessSendSPPData(TRUE);
                  ProcessSendLEKeyStateData();
                  break;
               ...
               case APPLICATION_MAILBOX_MESSAGE_ID_LE_CONNECTED:
                  ...
                  PostApplicationMailbox(APPLICATION_MAILBOX_MESSAGE_ID_KEY_STATE_CHANGE);
                  PostApplicationMailbox(APPLICATION_MAILBOX_MESSAGE_ID_ACCEL_CHANGE | APPLICATION_MAILBOX_MESSAGE_ID_X_AXIS | APPLICATION_MAILBOX_MESSAGE_ID_Y_AXIS | APPLICATION_MAILBOX_MESSAGE_ID_Z_AXIS);
               ...
               case APPLICATION_MAILBOX_MESSAGE_ID_LE_DISCONNECTED:
                  ...
                  BTPS_MemInitialize(&(ApplicationStateInfo.LEConnectionInfo), 0, sizeof(ApplicationStateInfo.CBConnectionInfo));
               ...
               case APPLICATION_MAILBOX_MESSAGE_ID_CB_CONNECTED:
                  ...
                  FormatAdvertisingData(ApplicationStateInfo.BluetoothStackID, FALSE);

                  /* Set stack to be non-discoverable/non-connectable as*/
                  /* we do only allow 1 BR/EDR Connection.              */
                  GAP_Set_Discoverability_Mode(ApplicationStateInfo.BluetoothStackID, dmNonDiscoverableMode, 0);
                  GAP_Set_Connectability_Mode(ApplicationStateInfo.BluetoothStackID, cmNonConnectableMode);
                  ...
                  ApplicationStateInfo.Flags |= APPLICATION_STATE_INFO_FLAGS_CB_CONNECTED;
               ...
               case APPLICATION_MAILBOX_MESSAGE_ID_CB_DISCONNECTED:
                  ...
                  GAP_Set_Discoverability_Mode(ApplicationStateInfo.BluetoothStackID, dmGeneralDiscoverableMode, 0);
                  GAP_Set_Connectability_Mode(ApplicationStateInfo.BluetoothStackID, cmConnectableMode);

                  /* Clear the BR/EDR Connection Information.           */
                  BTPS_MemInitialize(&(ApplicationStateInfo.CBConnectionInfo), 0, sizeof(ApplicationStateInfo.CBConnectionInfo));

                  /* Clear the BR/EDR Connection Flag and the SPP Buffer*/
                  /* Full Flag.                                         */
                  ApplicationStateInfo.Flags &= ~(APPLICATION_STATE_INFO_FLAGS_CB_CONNECTED | APPLICATION_STATE_INFO_FLAGS_SPP_BUFFER_FULL);
               ...
               default:
                  ...
                  /* Message.                                           */
                  if((MessageID & (~APPLICATION_MAILBOX_MESSAGE_ID_ACCEL_MASK)) == APPLICATION_MAILBOX_MESSAGE_ID_ACCEL_CHANGE)
                  {
